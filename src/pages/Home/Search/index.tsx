...
const Search = () => {
    ...
    const handleSubmit = (event: React.FormEvent<HTMLFormElement>) => {
        ...
        fetch('https://api.github.com/users/' + payload.path)
            .then(response => {
                response.ok ? setVisiblePerfil(true) : setVisiblePerfil(false);
                ...
                <div className="search-result-container">
                    {
                        isLoading ? <ImageLoader /> : (
                            <div className={`${visiblePerfil ? "search-result-col-3-v" : "search-result-col-3"}`}>
...

                    {
                        isLoading ? <InfoLoader /> : (
                            <div className={`${visiblePerfil ? "search-result-col-7-v" : "search-result-col-7"}`}>
